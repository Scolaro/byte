...
# Layout for the top half (Inputs)
...
with col1:
    with st.container(border=True):
        ...
        with r2c1:
            renovations_new = st.number_input("Geschätzte Reparaturen für das neue Haus (€)", value=25000, step=5000)
        with r2c2:
        ...
        # Pre-calculate actual monthly payment to display it here
        if loan_amount <= 0:
        ...
        st.markdown(f'<div class="calculated-result" style="margin-top: 10px; color: #0284c7;">Erforderliche monatliche Rate (Kredit): <b>€{format_eur(actual_monthly_payment)}</b></div>', unsafe_allow_html=True)
        ...
        # Instandhaltungsrücklage separate, taking up a 1/3 slot width-wise but placed alone on the row
...
with col2:
    with st.container(border=True):
        ...
        # To align the "Laufende Nebenkosten" sections, we add some spacing.
        # The new house has 4 rows of inputs for purchase/loan. We have 1 row for rent/renovations.
...
with col4:
    with st.container(border=True):
        ...
        st.markdown("---")
        st.subheader("Szenario A: Altes Haus ist vermietet")
